...
public class TFTPClient {
   ...
   private static final int MAX_PACKET_SIZE = 516;		//Maximum packet size in bytes
   ...
   private static enum Mode { NORMAL, TEST};		//Normal mode sends directly to the server, test mode sends to simulator
   private static enum Request { RRQ, WRQ, DATA, ACK, ERROR };	//All request types
   ...
   private void saveFileData(String filePath) {
	   ...
	   byte[] data = new byte[MAX_PACKET_SIZE];			//Byte array for packet data
   ...
   private void sendFileData(String filePath) {
	   ...
	   byte[] data = new byte[MAX_PACKET_SIZE];		//Byte array for file data being sent
	   byte[] dataSection = new byte[MAX_PACKET_SIZE - DATA_PACKET_HEADER_SIZE];//Size of the data in the packet
	   ...
	   try {
			...
			} while (bytesRead == MAX_PACKET_SIZE - DATA_PACKET_HEADER_SIZE);
